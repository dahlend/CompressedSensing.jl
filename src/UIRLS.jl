...
function UIRLS(MeasurementMatrix::AbstractArray{T,2},MeasuredOutput::AbstractArray{T,1};
    eps=x->1/x^3,threshold=1e-5,lambda=1,p=.5,verbose=false,maxiter=1000,debug=false,force_positive=false) where T
...
function UIRLS(MeasurementMatrix::Array{Float16,2},MeasuredOutput::Array{Float16,1};
    ...
    #set the distance between iterations to infinite
    PrevDist = fill(Inf,div(maxiter,100)+1)
